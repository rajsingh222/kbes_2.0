...
function getCurrencyForUser(user, req) {
  ...
  if (req) {
    ...
    if (detectedCountry) {
      ...
      console.log('  💵 Amount:', currency === 'INR' ? '₹100 (10000 paise)' : '$5 (500 cents)');
      return currency;
    }
  }
...
const razorpay = new Razorpay({
  key_id: process.env.RAZORPAY_KEY_ID || 'rzp_test_S3FBjTRnq3YpDM',
  key_secret: process.env.RAZORPAY_KEY_SECRET || 'u3im2pE9HEhFOLvC5R5L1m6b'
});
...
router.post('/create-order', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('  Amount:', amount, currency === 'INR' ? '(₹100)' : '($5)');
    console.log('  Display:', currency === 'INR' ? '₹100' : '$5');
    ...
    res.json({
      ...
      amount: amount,
      currency: currency,
      displayAmount: currency === 'INR' ? '₹100' : '$5',
      keyId: process.env.RAZORPAY_KEY_ID || 'rzp_test_S3FBjTRnq3YpDM'
    });
...
router.post('/verify-payment', authenticateToken, async (req, res) => {
  try {
    ...
    const expectedSignature = crypto
      .createHmac('sha256', process.env.RAZORPAY_KEY_SECRET || 'u3im2pE9HEhFOLvC5R5L1m6b')
